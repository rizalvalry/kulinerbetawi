                        ...
                        <div class="d-flex justify-content-center">
                            <template v-if="data.discount !== null && data.status !== 0 && data.expire_date > new Date().toISOString()">
                                <template v-if="data.type == 'Percentage'">
                                    ...
                                    <h6 >Rp. {{ Number(data.sale_price) }}</h6>
                                </template>
                                <template v-else>
                                    <h6 class="mr-4">Rp. {{ Number(data.sale_price) - Number(data.discount)  }}</h6>
                                    <h6 class="text-muted ml-2"><del>Rp. {{ Number(data.sale_price) }}</del></h6>
                                </template>
                            </template>
                            <template v-else>
                                <h6>Rp. {{ Number(data.sale_price) }}</h6>
